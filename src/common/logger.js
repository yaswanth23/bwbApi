...
const logger = winston.createLogger({
  ...
  transports: [
    ...
    new winston.transports.DailyRotateFile({
      name: "access-file",
      level: "info",
      filename: Os.homedir() + "/var/logs/sty-dev/bwb-api-info.log",
      json: false,
    ...
    new winston.transports.DailyRotateFile({
      name: "error-file",
      level: "error",
      filename: Os.homedir() + "/var/logs/sty-dev/bwb-api-error.log",
      json: false,
      datePattern: "yyyy-MM-DD",
      prepend: true,
      maxFiles: 10,
  ...
  return {
    ...
    responseLogger(req, res, next) {
      return mung.json(function transform(body, req, res) {
        const api = `https://${process.env.APP_PUBLIC}${req.originalUrl}`;
        logger.debug({
          api,
          module,
